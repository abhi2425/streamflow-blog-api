	...
	async ({ user, params }, res) => {
		try {
			...
			return res.status(200).send({
				message: `${user.name}'s '${deletedPost.title} post' deleted successfully!!`,
			})
		} catch (error) {
...
router.delete(
	...
	async ({ user, params }, res) => {
		try {
			await deleteImageFromCloudinary(params?.publicId)
			...
			if (postResult.nModified === 0)
				throw new Error('Could not finish the job!')
...
router.delete('/profile/posts/deleteAll', auth, async ({ user }, res) => {
	try {
		...
		if (posts.deletedCount === 0)
			throw new Error(`you don't have any post to delete!`)
		return res
			.status(200)
			.send({ message: `${user.name}'s all posts deleted successfully!!` })
	} catch (error) {
		res.status(404).send({ error: error.message })
	}
})
